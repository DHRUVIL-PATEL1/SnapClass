...
def teacher_dashboard():
  ...
  with col2:
    ...
    if st.button("Log Out", type="secondary", key="logoutbtn", shortcut="control+backspace", width="stretch"):
...
def teacher_tab_take_attendance():
  ...
  if not subjects:
    st.info("No subjects found! Create one to take attendance.")
    return
  ...
  with col2:
    if st.button("Add Photos", icon=":material/photo_prints:", width="stretch", type="primary"):
      add_photos_dialog()
...
def teacher_tab_manage_subjects():
  ...
  if subjects:
    for subject in subjects:
      # print(subject["total_students"])
      stats = [
        ("👥", "Students", subject["total_students"]),
        ("🕰️", "Classes", subject["total_classes"])
      ]
...
def teacher_tab_attendance_records():
  ...
  for r in records:
    ...
    data.append({
      ...
      "Subject": r["subjects"]["name"] if r["subjects"] else "-",
...
def teacher_screen_login():
  ...
  with col2:
    if st.button("Go back to Home", type="secondary", key="Homebtn1", shortcut="control+backspace"):
  ...
  teacher_username = st.text_input("Enter Username", placeholder="@someone")
...
def register_teacher(teacher_username, teacher_name, teacher_password, teacher_confirm_password):
  ...
  if teacher_password != teacher_confirm_password:
    return False, "Password doesn't match to confirm password"
...
# App Teacher Registration Screen
def teacher_screen_register():
  ...
  with col2:
    if st.button("Go back to Home", type="secondary", key="Homebtn2", shortcut="control+backspace"):
      st.session_state["login_type"] = None
      st.rerun()
  ...
  teacher_name = st.text_input("Enter Name", placeholder="Someone")

  teacher_password = st.text_input("Enter Password", type="password", placeholder="Enter Password")

  teacher_confirm_password = st.text_input("Confirm Password", type="password", placeholder="Confirm Password")
  ...
  with btnc2:
    if st.button("Login Instead", icon=":material/passkey:", width="stretch", type="secondary"):
      st.session_state["teacher_login_type"] = "login"
      st.rerun()
